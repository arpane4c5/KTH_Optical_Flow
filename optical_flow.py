...
# seq: dictionary containing frames numbers where actions occur
...
def construct_datasets(srcVideoFolder, setIDs, seq):
    ...
    videoCount = 1
    start_id = 0
    N = 400         # Approx number of OF frames per action video
    X_list = []
    #y_list = []
    ...
    for video in videosList:
        ...
        # get the key value from video name. Dict has corresponding string with 
        # frame sequences. Gets "person13_jogging_d4", removes "_uncomp.avi" .
        key = video.rsplit('_',1)[0]
        action_seq_str = seq[key]
        get_optical_flow_vid(os.path.join(srcVideoFolder, video), action_seq_str, X_list, key)
        ...
        with env.begin(write=True) as txn:
            while len(X_list)!=0:
                ...
                # randomize the key values by prepending a random int
                random.seed(i)
            ...
            # save the list in the LMDB incrementally, per video
            #start_id = convert_to_lmdb(os.path.join(srcVideoFolder, "LMDB", "OF_lmdb"), N, X_list, y_list, start_id)
...
def get_optical_flow_vid(srcVideo, action_seq_str, X_list, imgPrefix):
    srcVideo = "/home/hadoop/VisionWorkspace/KTH_OpticalFlow/dataset/kth_actions_train/person17_running_d4_uncomp.avi"
...
#                cv2.imwrite(os.path.join(os.getcwd(),imgPrefix+"_OFVecs_"+str(stime-1)+".jpg"), vis_vectors)
    ...
    plt.close("all")
    #createPlot(flow_means_x, 1, 'r',  "Horizontal Component")
    #createPlot(flow_means_y, 1, 'b', "Vertical Component")    
    createPlot(fgmask_sums, 1, 'b', "fgmask sums")    
    return

def createPlot(valuesList, fig_no, col, title_str):
    fig = plt.figure(fig_no)
    plt.title(title_str)
    plt.xlabel("Frame No.")
    plt.ylabel(title_str)
    plt.plot(valuesList, color=col, label=str(fig_no))
...
def draw_flow_bgr(flow, sample_frame, flow_means_x, flow_means_y):
    hsv = np.zeros_like(sample_frame)
    #print "hsv_shape : "+str(hsv.shape)
    hsv[...,1] = 255
    mag, ang = cv2.cartToPolar(flow[...,0], flow[...,1])
    print "Cartesian Flow Magnitude Stats : flow[...,0], flow[...,1] : "
    flow_means_x.append(np.sum(flow[...,0]))
    flow_means_y.append(np.sum(flow[...,1]))
    print "Mean : "+str(np.mean(flow[...,0]))+" :: SD : "+str(np.std(flow[...,0]))+" :: Sum :"+str(np.sum(flow[...,0]))
    
    #print "Cartesian Flow Angle Stats : flow[...,1] : "
    print "Mean : "+str(np.mean(flow[...,1]))+" :: SD : "+str(np.std(flow[...,1]))+" :: Sum :"+str(np.sum(flow[...,1]))
    #print "Polar Flow Magnitude Stats : mag : "
    #print mag
    #print "Mean : "+str(np.mean(mag))+" :: SD : "+str(np.std(mag))+" :: Sum :"+str(np.sum(mag))
    #print "Polar Flow Angle Stats : ang : "
    #print ang
    #print "Mean : "+str(np.mean(ang))+" :: SD : "+str(np.std(ang))
    
    hsv[...,0] = ang*180/np.pi/2
    hsv[...,2] = cv2.normalize(mag, None, 0, 255, cv2.NORM_MINMAX)
    print "HSV Mag values :"
    print "Mean : "+str(np.mean(hsv[...,2]))+" :: SD : "+str(np.std(hsv[...,2]))+" :: Sum :"+str(np.sum(hsv[...,2]))
...
def waitTillEscPressed():
    while(True):
        if cv2.waitKey(10)==27:
            print("Esc Pressed")
            return
        elif cv2.waitKey(10)==119:
            print("'w' pressed. Write to file")
            return 0
...
if __name__=="__main__":
    ...
    #construct_datasets(os.path.join(dataset, "kth_actions_test"), testPersons, seq_dict)
